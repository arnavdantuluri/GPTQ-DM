# Copied from: https://github.com/fpgaminer/GPTQ-triton
import functools
import math
...
def make_quant_lora(model, bits, groupsize):
	"""
	Replace all lora layers in a model with quantized ones.
	"""
	for name, m in model.named_modules():
		if not isinstance(m, LoRALinearLayer):
			continue

		# Replace the linear layer with a quantized one
		qlayer = QuantLoRA(bits, groupsize, m.in_features, m.out_features, m.bias is not None)
		parent_name = name.rsplit('.', 1)[0]
		parent = model.get_submodule(parent_name)
